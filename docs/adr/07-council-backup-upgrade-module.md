...
- **The delay is computed live** from the Governor's settings and the timelock's minimum, so a council upgrade always outlasts a full voter cancel cycle even if those settings change. The path is a **backup, never a fast-track**: slower than a voter proposal, valuable for liveness rather than speed.
