...
import { Claim } from "../src/MerkleDistributor.sol";
...
contract MockDistribution is MockVoters {
    ...
    uint256 public VOTE_SHARE = 1_000_000_000 / (2 * _TREE_DEPTH);
    ...
    constructor() {
        ...
        for (uint256 i = 0; i < 2 * _TREE_DEPTH; ++i) {
            bytes32 leaf = Claim.leafHash({ index: i, to: voter(i), value: VOTE_SHARE, lockedValue: VOTE_SHARE });
    ...
    function merkleProof(uint256 index) internal view returns (bytes32[] memory siblings, uint256 directionBits) {
