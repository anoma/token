...
/// upgrades it to `XanV2` via the production voter-body upgrade flow. The vesting schedule defaults to the
/// `Parameters` values; override `_vestingSchedule` to customize (e.g. to place the start after the upgrade).
abstract contract XanV2Fixture is Test {
    ...
    /// @notice The vesting `(start, duration)` baked into the deployed V2 implementation. Override to customize.
